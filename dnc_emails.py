...
DNC_DIR  = os.path.join(HOME, "emails")
SLEEP = 20
CHUNK_TO_SLEEP = 100
...
def main():
	...
	parser.add_argument("--async", dest="async", default= ASYNC, 
						help="1, 0 for use of async request or not respectively. Use this if you dont care about the order of the requests",
						 default=0)
	...
	for (i, r) in zip(xrange(1, MAX_EMAILS+1), resp):
		if r.ok:
			...
			fail_count = 0
			json.dump({"data" : r.text}, open(os.path.join(DNC_DIR, "_" + str(i) + ".json"), "w"))
		else:
